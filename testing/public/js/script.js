...
function fetchData() {
  ...
  const urlRocket = "https://ll.thespacedevs.com/2.2.0/launch/upcoming/?limit=16";
...
*/
      ...
      function updateCountdown() {
        ...
        const timeRemaining = launchTime - now;
          ...
          let countdownText = `<div id="czasElement">${formattedDays}:${formattedHours}:${formattedMinutes}:${formattedSeconds}</div>`;

          if (days === 1) {
            countdownText = `<div id="czasElement">${formattedDays}:${formattedHours}:${formattedMinutes}:${formattedSeconds}</div>`;
          } if (days > 2) {
            countdownText = `<div id="czasElement">Start za ${days} dni</div>`;
          } 

          if (statusElement.innerHTML !== "DATA POTWIERDZONA") {
            countdownText = `<div id="czasElement">${status}</div>`;
          }
